...
func (s *Service) GetSession(ctx context.Context, userID string, params repository.GetSessionParams) (*repository.PaginatedSessionDetail, error) {
	if err := s.checkRBAC(ctx, userID, params.Query.OrganizationID); err != nil {
		return nil, err
	}

	// Apply defaults with page size of 50 for transcripts
	...
	session, err := s.repo.GetSession(ctx, params)
	if err != nil {
		s.logger.Error("failed to get session",
			slog.String("organizationID", params.Query.OrganizationID),
			slog.String("sessionID", params.Query.SessionID),
			slog.Int("page", int(params.Page)),
			slog.Int("pageSize", int(params.PageSize)),
			slog.Any("error", err),
		)
