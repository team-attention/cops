...
// Method name matches protobuf-generated interface (uses 'Id' per protobuf convention).
func (h *ProjectGRPCHandler) GetProjectByIdAndOrg(
	ctx context.Context,
	req *connect.Request[projectv1.GetProjectByIdAndOrgReq],
) (*connect.Response[projectv1.GetProjectByIdAndOrgRes], error) {
	// Extract projectID and organizationID from request
	msg := req.Msg
	projectID := msg.GetProjectId()
	organizationID := msg.GetOrganizationId()

	// Call service (Go naming)
	result, err := h.svc.GetProjectByIDAndOrg(ctx, projectID, organizationID)
	if err != nil {
		return nil, connect.NewError(connect.CodeInternal, err)
	}

	// Build response from service result
	res := &projectv1.GetProjectByIdAndOrgRes{
		Found:          result.Found,
		ProjectId:      result.ProjectID,
		Name:           result.Name,
		OrganizationId: result.OrganizationID,
