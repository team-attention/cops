...
func (r *MongoProjectRepository) FindOrCreate(ctx context.Context, params repository.FindOrCreateParams) (*repository.FindOrCreateResult, error) {
	...
	if len(conditions) > 0 {
		project, err := r.findByConditions(ctx, params.OrganizationID, conditions)
		if err != nil {
			return nil, err
		}
		if project != nil {
...
// findByConditions executes the $or query with optional OrganizationID filter.
func (r *MongoProjectRepository) findByConditions(ctx context.Context, orgID string, conditions []bson.M) (*repository.FindOrCreateResult, error) {
	filter := bson.M{"$or": conditions}

	// Add organizationId filter if provided
	if orgID != "" {
		if orgObjID, err := bson.ObjectIDFromHex(orgID); err == nil {
			filter[mongoschema.ProjectOrganizationIDField] = orgObjID
		}
	}
...
func (r *MongoProjectRepository) GetByIDAndOrg(ctx context.Context, projectID string, organizationID string) (*repository.GetByIDAndOrgResult, error) {
	// Parse projectID string to bson.ObjectID
	projObjID, err := bson.ObjectIDFromHex(projectID)
	if err != nil {
		return nil, fmt.Errorf("invalid project ID format: %w", err)
	}

	// Parse organizationID string to bson.ObjectID
	orgObjID, err := bson.ObjectIDFromHex(organizationID)
	if err != nil {
		return nil, fmt.Errorf("invalid organization ID format: %w", err)
	}

	// Query with compound filter: _id and organizationId
	filter := bson.M{
		mongoschema.ProjectIDField:             projObjID,
	...
	if err == mongo.ErrNoDocuments {
		return &repository.GetByIDAndOrgResult{Found: false}, nil
	}
	if err != nil {
		r.logger.Error("failed to find project by ID and org",
			slog.String("projectID", projectID),
			slog.String("organizationID", organizationID),
			slog.Any("error", err))
		return nil, fmt.Errorf("failed to find project by ID and org: %w", err)
	}

	// Decode document and populate result
	result := &repository.GetByIDAndOrgResult{
		Found:     true,
		ProjectID: doc[mongoschema.ProjectIDField].(bson.ObjectID).Hex(),
		Name:      doc[mongoschema.ProjectNameField].(string),
	}
