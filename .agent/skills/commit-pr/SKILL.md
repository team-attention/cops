...
Analyze the `git status` output and the Artifact content to identify which files belong to the current task scope.
Stage the files relevant to the task (e.g. source code, tests, configs) AND the specific artifact directory generated for this session.

```bash
git add <list_of_relevant_files> .agent/artifacts/$ARTIFACT_ID
```
