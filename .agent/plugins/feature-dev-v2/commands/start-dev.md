...
| --- | ----------- | ---------------------- | ------ | -------------- | -------- | -------------------------- |
| 0   | Initialize  | **artifact skill**     | -      | `$ARTIFACT_ID` | No       | Generate artifact ID       |
| 1   | Clarify     | **clarify skill**      | gemini | requirements   | Yes      | Gather requirements        |
| 2   | Plan        | planning               | opus   | plan           | Yes      | Design implementation      |
| 3   | Execute     | execute                | opus   | (uses plan)    | No       | Implement code             |
| 4   | Review      | **review-cycle skill** | -      | -              | No*      | Auto-loop handled by skill |
| 5   | Walkthrough | **walkthrough skill**  | gemini | walkthrough    | Yes      | Document changes           |
| 6   | Commit & PR | **commit-pr skill**    | gemini | -              | No       | Auto commit and PR         |
...
5. **If "No, needs changes"**:
   ...
   - Resume clarify skill with feedback (append feedback to request if needed, or re-run):
     ```
     Use Skill tool:
     - skill: clarify
     - args: "Original request: $ARGUMENTS. Feedback: [user feedback]" $CLARIFY_FILE
     ```
...
2. Invoke walkthrough agent:
   ...
   Use Skill tool:
   - skill: walkthrough
   - args: $ARTIFACT_ID $WALKTHROUGH_FILE
   ```
   ```
