...
func newTrackingModule(c *dig.Container) error {
	...
	if err := c.Provide(
		provideFilesystemConfigAdapter,
...
// provideFilesystemConfigAdapter wraps the tracking config adapter constructor.
func provideFilesystemConfigAdapter(l *slog.Logger, paths *pathutil.ExpandedPaths) (*filesystem.FilesystemConfigAdapter, error) {
	return filesystem.NewFilesystemConfigAdapter(l, paths.GlobalConfigPath, paths.LocalConfigDir)
}
