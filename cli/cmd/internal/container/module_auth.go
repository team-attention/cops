...
func newAuthModule(c *dig.Container) error {
	...
	if err := c.Provide(
		provideFilesystemAPIKeyStorage,
...
func provideFilesystemAPIKeyStorage(l *slog.Logger, paths *pathutil.ExpandedPaths) *filesystemadapter.FilesystemAPIKeyStorage {
	return filesystemadapter.NewFilesystemAPIKeyStorage(l, paths.AuthPath)
}
