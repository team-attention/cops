...
// Parameters:
//   - l: Logger instance for structured logging
//
// Returns:
//   - apikey.APIKeyPort: The adapter implementing the port interface
func NewFilesystemAPIKey(l *slog.Logger) apikey.APIKeyPort {
	...
	// 3. Create and return FilesystemAPIKey with logger binding.
...
// FilesystemAPIKeyWithPath is an alias for FilesystemAPIKey that exposes
// the struct for testing with custom paths.
type FilesystemAPIKeyWithPath = FilesystemAPIKey

// NewFilesystemAPIKeyWithPath creates a FilesystemAPIKey with a custom auth path.
// This is primarily used for testing to avoid affecting real config files.
// Parameters:
//   - l: Logger instance for structured logging
//   - authPath: Custom path to the auth.json file
//
// Returns:
//   - *FilesystemAPIKeyWithPath: The adapter with custom path
func NewFilesystemAPIKeyWithPath(l *slog.Logger, authPath string) *FilesystemAPIKeyWithPath {
	return &FilesystemAPIKeyWithPath{
		logger:   l.With(slog.String("name", "platform.apikey.filesystem")),
		authPath: authPath,
	}
}
