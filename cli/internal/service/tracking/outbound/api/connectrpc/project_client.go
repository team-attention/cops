...
func (c *ProjectClient) GetProjectByIDAndOrg(ctx context.Context, accessToken string, projectID string, organizationID string) (*api.GetProjectByIDAndOrgResult, error) {
	req := connect.NewRequest(&projectv1.GetProjectByIdAndOrgReq{
		ProjectId:      projectID,
		OrganizationId: organizationID,
	})
	req.Header().Set("Authorization", "Bearer "+accessToken)

	resp, err := c.client.GetProjectByIdAndOrg(ctx, req)
	if err != nil {
		c.logger.Error("failed to get project by ID and org",
	...
	result := &api.GetProjectByIDAndOrgResult{
		Found:          resp.Msg.Found,
		ProjectID:      resp.Msg.ProjectId,
		Name:           resp.Msg.Name,
		OrganizationID: resp.Msg.OrganizationId,
	}
