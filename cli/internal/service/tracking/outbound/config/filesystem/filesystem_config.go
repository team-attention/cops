...
func NewFilesystemConfigAdapter(l *slog.Logger, globalConfigPath, localConfigDir string) (*FilesystemConfigAdapter, error) {
	logger := l.With(slog.String("name", "tracking.config.filesystem"))

	// Ensure directory exists
	if err := pathutil.EnsureDir(filepath.Dir(globalConfigPath)); err != nil {
		return nil, err
	}
...
// SaveLocalConfig saves the local configuration to {projectPath}/{localConfigDir}/config.json.
func (a *FilesystemConfigAdapter) SaveLocalConfig(projectPath string, cfg *config.LocalConfig) error {
	configDir := filepath.Join(projectPath, a.localConfigDir)
...
// DeleteLocalConfig deletes the local configuration directory at the given path.
